...
filter ::
  ...
  List a ->
  List a
filter f = foldRight (\a memo -> if f a then a :. memo else memo) Nil
...
seqOptional (a :. ax) = case a of
  Empty -> Empty
  Full val -> case seqOptional ax of
    Empty -> Empty
    Full vals -> Full (val :. vals)
...
find ::
  ...
  Optional a
find f Nil = Empty
find f (a :. ax) = if f a then Full a else find f ax
